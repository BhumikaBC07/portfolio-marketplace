  ...
  return (
      <Float speed={1.2} rotationIntensity={0.5} floatIntensity={1}>
        <mesh ref={mesh} position={position} scale={scale}>
          ...
          <meshStandardMaterial color="#c8bcac" transparent opacity={0.4} roughness={0.5} />
        </mesh>
      </Float>
  );
}
...
function Scene() {
  ...
  return (
      <>
        ...
        {shapes.map((shape, i) => {
          if (shape.type === 'plane') return <FloatingPlane key={i} position={shape.position} rotation={shape.rotation} scale={shape.scale} />;
          if (shape.type === 'box')   return <FloatingBox   key={i} position={shape.position} rotation={shape.rotation} scale={shape.scale} />;
